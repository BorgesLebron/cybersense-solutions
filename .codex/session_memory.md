...
## 2026-05-18/19 (Gwen - Intel Article Production Path)

### Scope
- Applied the Threat Radar hover summary behavior to the Ops Innovation/Growth intake table in `api_layer/public/admin.html`.
- Added James as the missing Intel Article acquisition writer runtime.
- Extended article downstream runtime coverage so Intel Articles can move beyond Rob:
  - Jeff now handles `qa_article` tasks.
  - Maya now handles `approve_article` tasks.
- Updated scheduler wiring for James, Jeff, and the James article cycle trigger.

### Implementation
- `api_layer/services/james_runtime.js` drafts one article from a ready-for-intel repository item without a linked article.
- James uses env fallback order:
  - `JAMES_BRAIN_API_KEY`
  - `INTEL_EDITORIAL_BRAIN_API_KEY`
  - `EDITORIAL_BRAIN_API_KEY`
  - `GOOGLE_GENERATIVE_AI_API_KEY`
- James submits drafts to `/api/pipeline/articles`; the existing pipeline queues Jason automatically.
- `runJamesArticleCycle()` creates one James `draft_article` task when:
  - no James draft task is already active,
  - fewer than 3 articles are currently in the pipeline,
  - at least one ready-for-intel repository item has no linked article.
- Scheduler now polls:
  - James every 2 minutes.
  - Jeff every 2 minutes.
  - Existing Jason/Rob/Maya article handoff remains in place.

### Validation
- `node --check` passed for James, Jeff, Maya, scheduler, and Intel editorial tests.
- Parsed inline scripts from `api_layer/public/admin.html`; passed.
- `git diff --check` passed for touched files.
- `npm.cmd test` passed: 4 suites / 23 tests.
